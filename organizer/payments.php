...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        // Fetch the payment proof details, ensuring it belongs to this organizer's event
        $stmt = $pdo->prepare("SELECT p.*, b.quantity, b.ticket_type_id, b.event_id, e.title as event_title, u.id as attendee_id, u.name as attendee_name, u.telegram_chat_id
                               ...
                               WHERE p.id = ? AND e.organizer_id = ? FOR UPDATE");
        $stmt->execute([$proof_id, $organizer['id']]);
        $proof = $stmt->fetch();

        if (!$proof) {
            $error = "Payment proof record not found or access denied.";
            ...
            if ($action === 'approve') {
                ...
                $up_proof->execute([$organizer['id'], $proof_id]);
                ...
                $pdo->commit();
                $success = "Payment proof #{$proof_id} approved successfully! Ticket is now generated.";

                // Notify attendee
                $notif_msg = "Your payment of ₹{$proof['amount']} for event '{$proof['event_title']}' has been verified. Your ticket is now ready! You can view and download it from your dashboard.";
                ...
                $pdo->commit();
                $success = "Payment proof #{$proof_id} rejected. Seats have been restored.";

                // Notify attendee
                $reason_suffix = !empty($rejection_reason) ? " Reason: " . $rejection_reason : "";
                $notif_msg = "Your payment proof for event '{$proof['event_title']}' was rejected because the transaction could not be verified (UTR: {$proof['utr_number']}).{$reason_suffix} Please contact organizer for assistance.";
...
// Fetch all payment proofs for events owned by this organizer
try {
    $stmt = $pdo->prepare("SELECT p.*, b.status as booking_status, b.total_price, b.quantity, u.name as attendee_name, u.email as attendee_email, e.title as event_title
                           FROM payment_proofs p
                           JOIN bookings b ON p.booking_id = b.id
                           JOIN users u ON p.user_id = u.id
                           JOIN events e ON b.event_id = e.id
    ...
    <div class="col-md-3 mb-4 no-print">
        <div class="glass-panel p-3">
            ...
            <div class="d-flex flex-column gap-1">
                ...
                <a href="payments.php" class="sidebar-link active"><i class="bi bi-credit-card"></i> Verify Payments</a>
    ...
    <div class="col-md-9">
        <div class="glass-panel p-4">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold text-white mb-1">Verify Payments</h4>
                    <p class="text-secondary mb-0 small">Approve booking tickets after checking transaction details and proof screenshot</p>
                </div>
            </div>
            ...
            <?php if (empty($payments)): ?>
                <div class="text-center py-5">
                    ...
                    <p class="text-secondary small">When attendees book tickets via UPI, their transaction receipts will appear here for verification.</p>
                </div>
            <?php else: ?>
                            ...
                            <?php foreach ($payments as $p): ?>
                                <tr style="border-bottom: 1px solid var(--glass-border);">
                                    <td>
                                        ...
                                        <div class="text-muted small">Booking: #<?php echo $p['booking_id']; ?> (Qty: <?php echo $p['quantity']; ?>)</div>
                                    </td>
                                    <td>
